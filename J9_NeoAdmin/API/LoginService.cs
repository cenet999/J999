...
using RestSharp;
using J9_NeoAdmin.API.DTOs;
using LoginRequest = J9_NeoAdmin.API.DTOs.LoginRequest;
...
public class LoginService : BaseService
{
    ...
    public LoginService(IFreeSql freeSql, Scheduler scheduler, ILogger<LoginService> logger, IConfiguration configuration, TGMessageApi TGMessageApi, IWebHostEnvironment webHostEnvironment, NeoAdminAuthService authService)
...
""";
    ...
    {
        HttpContext?.Response.Cookies.Delete("NeoAdmin.Auth" + "_" + tenantId);
    ...
    public async Task<ApiResult> InitDbData()
    {
        ...
        var taskCount = await _fsql.Select<J9_NeoAdmin.Entities.DTask>().CountAsync();
        if (taskCount == 0)
        {
            using var uow = _fsql.CreateUnitOfWork();
            var tasks = new List<J9_NeoAdmin.Entities.DTask>
            {
                new J9_NeoAdmin.Entities.DTask { Title = "每日登录", TaskType = "Login", TargetValue = 1, RewardAmount = 1m, ActivityPoint = 20, Description = "每天首次登录系统领取", Icon = "flame", JumpPath = "", IsEnabled = true, Sort = 1, CreatedTime = DateTime.Now },
                new J9_NeoAdmin.Entities.DTask { Title = "每日签到", TaskType = "CheckIn", TargetValue = 1, RewardAmount = 1m, ActivityPoint = 20, Description = "完成每日签到打卡", Icon = "calendar-check", JumpPath = "", IsEnabled = true, Sort = 2, CreatedTime = DateTime.Now },
                new J9_NeoAdmin.Entities.DTask { Title = "每日充值", TaskType = "Recharge", TargetValue = 100, RewardAmount = 5m, ActivityPoint = 30, Description = "每日充值>100元可领取", Icon = "coins", JumpPath = "/trans/recharge", IsEnabled = true, Sort = 3, CreatedTime = DateTime.Now },
                new J9_NeoAdmin.Entities.DTask { Title = "参与游戏", TaskType = "PlayGame", TargetValue = 5, RewardAmount = 2m, ActivityPoint = 30, Description = "每日累计参与5局游戏", Icon = "star", JumpPath = "/game/list", IsEnabled = true, Sort = 4, CreatedTime = DateTime.Now },
                new J9_NeoAdmin.Entities.DTask { Title = "邀请好友", TaskType = "Invite", TargetValue = 1, RewardAmount = 10m, ActivityPoint = 20, Description = "成功邀请1位好友注册", Icon = "star", JumpPath = "/user/invite", IsEnabled = true, Sort = 5, CreatedTime = DateTime.Now }
            };
            await uow.GetRepository<J9_NeoAdmin.Entities.DTask>().InsertAsync(tasks);
            uow.Commit();
        }
    ...
    {
        var titles = await _fsql.Select<SysSiteSettings>()
            .Where(t => t.IsEnabled)
            .ToListAsync(t => new
            {
                Id = t.Id.ToString(),
